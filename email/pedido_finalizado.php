...
/**
 * Enviar notificación de pedido finalizado al cliente.
 * @param string $correoDestino
 * @param string $fechaRecogida  Fecha que se mostrará al cliente (formato libre, viene desde la ventana emergente)
 * @param array|null $detalle  Array con los detalles del pedido (Id_pedido, Cantidades, Centimetros, Costo, Fecha_Entrega, Estado_Pedido, NombreCliente) y opcionalmente 'Imagenes' => [ ['url','cantidad','nombre'], ... ]
 * @return bool
 */
function enviarCorreoPedidoFinalizado($correoDestino, $fechaRecogida, $detalle = null) {
    ...
    try {
        ...
        $mail->Subject = 'Su pedido ha sido finalizado';

        $mensaje = '';
        $mensaje .= '<div style="font-family: Arial, Helvetica, sans-serif; color: #222; background: #f6f7f9; padding: 18px;">';
        $mensaje .= '<p style="margin:0 0 12px; font-size:15px; color:#333;">Su pedido ha sido finalizado correctamente. A continuación encontrará los detalles y las imágenes asociadas.</p>';

        if ($fechaRecogida) {
            $mensaje .= '<p style="margin:0 0 12px; font-size:15px; color:#1f3b73;"><strong>Puede recoger su pedido en la siguiente fecha:</strong> ' . htmlspecialchars($fechaRecogida) . '</p>';
        }
        ...
        if ($detalle && is_array($detalle)) {
            ...
            // imágenes: preferimos recibirlas en $detalle['Imagenes'] con la estructura esperada
            $imgItems = [];
            ...
            $mensaje .= '</div>'; // cierre card detalle
        }
